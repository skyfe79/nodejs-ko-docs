...
- [Don't Block the Event Loop (or the Worker Pool)](#dont-block-the-event-loop-or-the-worker-pool)
  ...
  - [Node.js 간단 리뷰](#nodejs-간단-리뷰)
    - [이벤트 루프에서 실행되는 코드는 무엇인가?](#이벤트-루프에서-실행되는-코드는-무엇인가)
    - [Worker Pool에서 실행되는 코드는 무엇인가요?](#worker-pool에서-실행되는-코드는-무엇인가요)
  ...
  - [이벤트 루프를 블로킹하지 마세요](#이벤트-루프를-블로킹하지-마세요)
    ...
    - [이벤트 루프 블로킹: REDOS](#이벤트-루프-블로킹-redos)
      - [취약한 정규 표현식 피하기](#취약한-정규-표현식-피하기)
      - [REDOS 예제](#redos-예제)
      - [Anti-REDOS 리소스](#anti-redos-리소스)
    - [이벤트 루프 블로킹: Node.js 코어 모듈](#이벤트-루프-블로킹-nodejs-코어-모듈)
    - [이벤트 루프 블로킹: JSON DOS](#이벤트-루프-블로킹-json-dos)
    - [이벤트 루프를 블로킹하지 않고 복잡한 계산 수행하기](#이벤트-루프를-블로킹하지-않고-복잡한-계산-수행하기)
      - [파티셔닝](#파티셔닝)
      - [오프로딩](#오프로딩)
        - [작업 오프로딩 방법](#작업-오프로딩-방법)
        - [오프로딩의 단점](#오프로딩의-단점)
        - [작업 분산을 위한 몇 가지 제안](#작업-분산을-위한-몇-가지-제안)
      - [오프로딩: 결론](#오프로딩-결론)
  - [Worker Pool을 블로킹하지 마세요](#worker-pool을-블로킹하지-마세요)
    - [작업 시간의 변동 최소화](#작업-시간의-변동-최소화)
      - [변형 예제: 오래 걸리는 파일 시스템 읽기](#변형-예제-오래-걸리는-파일-시스템-읽기)
      - [변형 예제: 오래 걸리는 암호화 작업](#변형-예제-오래-걸리는-암호화-작업)
    - [작업 분할](#작업-분할)
    - [태스크 분할 피하기](#태스크-분할-피하기)
    - [Worker Pool: 결론](#worker-pool-결론)
